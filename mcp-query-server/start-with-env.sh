...
# MCP Query Server Startup Script with Environment Variables
...
echo -e "${BLUE}🚀 Starting MCP Query Server with Environment Configuration${NC}"

# Function to find the current JAR file
find_current_jar() {
    local target_dir="$SCRIPT_DIR/target"
    local jar_pattern="mcp-query-server-*.jar"
...
kill_existing_servers() {
    ...
    local jar_name="mcp-query-server"
    local existing_pids=$(ps aux | grep "$jar_name" | grep -v grep | awk '{print $2}' || true)
    
    if [ -n "$existing_pids" ]; then
        echo -e "${YELLOW}⚠️ Found existing MCP query server processes:${NC}"
        ...
        echo -e "${GREEN}✓ Existing MCP query server processes terminated${NC}"
    else
        echo -e "${GREEN}✓ No existing MCP query server processes found${NC}"
    fi
...
echo -e ""
echo -e "${GREEN}🚀 Starting MCP Query Server...${NC}"
