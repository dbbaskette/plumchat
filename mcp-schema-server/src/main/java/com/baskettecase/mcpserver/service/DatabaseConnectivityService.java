...
public class DatabaseConnectivityService {
    ...
    public void checkDatabaseConnectivity() {
        logger.info("🚀 MCP Schema Server started successfully!");
        logger.info("🌐 Server running on port: {}", environment.getProperty("server.port", "8080"));
        logger.info("📡 MCP tools endpoints available at: http://localhost:{}", environment.getProperty("server.port", "8080"));
        ...
        try (Connection connection = dataSource.getConnection()) {
            ...
            try (ResultSet schemas = metaData.getSchemas()) {
                ...
                if (schemaCount > 0) {
                    logger.info("✅ Schema discovery working - found {} schemas", schemaCount);
                    logger.info("🛠️  MCP tools are ready for database operations!");
                } else {
            ...
            logger.warn("⚠️  MCP tools may not function properly without database connectivity");
        ...
        logger.info("🌟 MCP Schema Server initialization complete!");
